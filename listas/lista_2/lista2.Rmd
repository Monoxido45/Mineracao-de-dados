...
# calculando erro padrao para cada metodo
medidas$IC_lower = medidas$risco_krr - errors
medidas$IC_upper = medidas$risco_krr + errors
medidas
```
Vemos um ajuste melhor que o da floresta aleatória, com um intervalo de confiança também menor. Por fim, podemos plotar os valores preditos versus os observados para cada modelo como a seguir:
```{r}
# juntando todas as predicoes
all_preds = data.frame(resp = rep(y_test, 6),
                       preds = c(preds, as.numeric(pred.lasso), preds_rf$predictions,
                                preds_xgb, as.numeric(preds_nn), preds_krr),
                       mods = c(rep("KNN", length(y_test)), rep("Lasso", length(y_test)),
                                rep("Floresta aleatórias", length(y_test)),
                                rep("Xgboost", length(y_test)),
                                rep("Rede neural", length(y_test)),
